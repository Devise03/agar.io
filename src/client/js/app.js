...
function setupSocket(socket) {
    ...
    socket.on('playerScore', function (data) {
        document.getElementById('score').innerHTML = 'Score: ' + Math.round(data.score);
    });
    ...
    socket.on('serverTellPlayerMove', function (userData, foodsList, massList) {
        ...
        if(playerType == 'player') {
            ...
            document.getElementById('player-position').innerHTML = 'X:' + parseInt(player.x) + ' Y:' + parseInt(player.y);
            drawmap();
        }
        users = userData;
        foods = foodsList;
...
function drawmap() {
    ...
    map_ctx.width = map_canvas.width;
    map_ctx.height = map_canvas.height;

    map_ctx.lineWidth = 0.4;
    map_ctx.strokeStyle = lineColor;
    map_ctx.globalAlpha = 1;
    map_ctx.beginPath();

    for (var x = 0; x <= map_ctx.width; x += map_ctx.width / 10) {
        map_ctx.moveTo(x, 0);
        map_ctx.lineTo(x, map_ctx.height);
    }

    for (var y = 0; y <= map_ctx.height; y += map_ctx.height / 10) {
        map_ctx.moveTo(0, y);
        map_ctx.lineTo(map_ctx.width, y);
    }

    map_ctx.stroke();
    map_ctx.globalAlpha = 1;

    var playerX = player.x * map_ctx.width / gameWidth;
    var playerY = player.y * map_ctx.height / gameHeight;
    map_ctx.beginPath();
    map_ctx.arc(playerX, playerY, 5, 0, 2 * Math.PI);
    map_ctx.fillStyle = 'hsl(' + player.hue + ', 100%, 50%)';
    map_ctx.fill();
    map_ctx.stroke();
}
